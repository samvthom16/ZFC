	...
	<?php $index = 0;while( $this->query->have_posts() ) : $this->query->the_post();?>
		...
		<div class="item <?php if ($index == 0) { echo ' active'; } ?>" style="background-image:url('<?php echo $image; ?>')">
			<div class="carousel-caption">
				...
				<h2>
					<?php the_title_attribute(); ?></h2>
					<?php the_excerpt(); ?>
					<h1><a href="<?php the_permalink(); ?>" class="btn-lg btn btn-default" rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>">Read More</a></h1>
			</div>
		</div>
